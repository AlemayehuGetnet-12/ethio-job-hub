...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await axios.post(getApiUrl('/api/auth/login'), data, { withCredentials: true });
      setLoading(false);
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto flex min-h-screen max-w-3xl flex-col justify-center px-6 py-12">
        <div className="rounded-3xl border border-slate-800 bg-slate-900 p-10 shadow-xl shadow-slate-950/20">
          ...
          <p className="mt-3 text-slate-400">Access your job seeker or employer dashboard in EthioJobs Connect.</p>
          ...
          <div className="mt-6">
            {/* Telegram login widget: requires NEXT_PUBLIC_TELEGRAM_BOT_USERNAME to be set */}
            <div className="mb-3 text-sm text-slate-400">Or sign in using Telegram</div>
          ...
          <div className="mt-4 flex flex-col gap-2 text-sm text-slate-400">
            ...
            <p>
              New to EthioJobs? <Link href="/register" className="text-white underline">Create an account</Link>.
            </p>
          </div>
        </div>
      </div>
